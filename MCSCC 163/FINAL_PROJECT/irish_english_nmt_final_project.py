...
def load_eubookshop_dataset():
    ...
    try:
            ...
            if split in dataset:
                for item in dataset[split]:
                    if 'ga' in item and 'en' in item:
                        ...
                        # Validate that both sentences are non-empty
                        if irish_text and english_text and len(irish_text.strip()) > 0 and len(english_text.strip()) > 0:
                            irish_sentences.append(irish_text)
                            english_sentences.append(english_text)
        
        print(f"Loaded {len(irish_sentences)} parallel sentences")
        
        if len(irish_sentences) == 0:
            print("Warning: No valid sentences found in dataset, falling back to synthetic dataset")
        ...
        # Validate that preprocessing didn't empty the lists
        if len(processed_irish) == 0 or len(processed_english) == 0:
            print("Warning: Preprocessing resulted in empty lists, falling back to synthetic dataset")
            return create_synthetic_dataset(50000)
...
class BPEOptimizer:
    ...
    def _analyze_tokenization_efficiency(self, tokenizer, ga_samples, en_samples):
        ...
        if len(ga_samples) == 0 or len(en_samples) == 0:
            logger.warning("Empty samples provided for tokenization efficiency analysis")
            return {
                'avg_tokens_ga': 0,
                'avg_tokens_en': 0,
                'avg_chars_ga': 0,
        ...
        # Safeguard against division by zero in compression ratio
        compression_ratio = 0
        if avg_tokens_ga > 0 and avg_tokens_en > 0:
            compression_ratio = (avg_chars_ga / avg_tokens_ga + avg_chars_en / avg_tokens_en) / 2
        else:
            logger.warning("Zero tokens found, cannot compute compression ratio")
        
        logger.info(f"Tokenization efficiency: GA tokens={avg_tokens_ga:.2f}, EN tokens={avg_tokens_en:.2f}, compression={compression_ratio:.2f}")
...
class Evaluator:
    ...
    def evaluate_bleu(self, num_samples=1000):
        """
        Calculate BLEU score on test set with comprehensive error handling
        """
        try:
            import sacrebleu
        except ImportError:
        ...
        if not self.test_dataset or len(self.test_dataset) == 0:
            logger.error("Test dataset is empty, cannot evaluate BLEU")
            return 0.0, [], []
        
        # Safeguard against requesting more samples than available
        actual_samples = min(num_samples, len(self.test_dataset))
        test_subset = self.test_dataset[:actual_samples]
        
        logger.info(f"Starting BLEU evaluation on {actual_samples} samples...")
        
        valid_samples = 0
        for i, (src, ref) in enumerate(test_subset):
            if i % 100 == 0:
                logger.info(f"Processed {i}/{actual_samples} samples...")
            
            try:
                # Generate translation
                hyp = self.beam_decoder.decode(src)
                
                # Clean special tokens and validate
        ...
        logger.info(f"Successfully processed {valid_samples}/{actual_samples} samples for BLEU")
        
        # Safeguard against empty references or hypotheses
        ...
        try:
            ...
            return 0.0, references, hypotheses
    ...
    def error_analysis(self, references, hypotheses):
        """
        Analyze common error patterns with safeguards against empty data
        """
        print("\nError Analysis:")
        print("=" * 50)
        ...
        # Safeguard against empty references or hypotheses
        if not references or not hypotheses:
            print("Warning: Empty references or hypotheses list - cannot perform error analysis")
            print(f"References count: {len(references)}, Hypotheses count: {len(hypotheses)}")
            return
        
        # Safeguard against mismatched lengths
        if len(references) != len(hypotheses):
            print(f"Warning: Mismatched lengths - references: {len(references)}, hypotheses: {len(hypotheses)}")
        ...
        exact_matches = sum(1 for ref, hyp in zip(references, hypotheses)
                          if ref and ref[0] and hyp and ref[0] == hyp)
        
        # Safeguard against division by zero
        if len(references) == 0:
            print("Error: No references available for analysis")
            return
        ...
        # Analyze length patterns with safeguards
        ref_lengths = []
        hyp_lengths = []
        ...
        for hyp in hypotheses:
            if hyp:
                hyp_lengths.append(len(hyp.split()))
        
        # Safeguard against empty length lists
        if ref_lengths and hyp_lengths:
            ...
            # Safeguard against division by zero for length ratio
            if avg_ref_len > 0:
                print(f"Length ratio: {avg_hyp_len/avg_ref_len:.2f}")
            else:
                print("Length ratio: N/A (zero reference length)")
        else:
            print("Warning: No valid reference or hypothesis lengths for analysis")
